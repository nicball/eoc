    ...
    (define/override (type-check-exp env)
      (lambda (e)
        ...
        (match e
          ...
          [(HasType e1 T)
           (define-values (e1^ T1) (recur e1))
           (check-consistent? T1 T e)
           (values (make-cast e1^ T1 T) T)]          
          [(Cast e src tgt)
           (define-values (e^ t) (recur e))
           (check-consistent? src t e)
           (values (Cast e^ src tgt) tgt)]
          [(Apply e1 e2s)
           (define-values (e1^ T1) (recur e1))
...
(define (type-check-Lany-proxy-mixin super-class)
  (class super-class
    ...
    (define/override ((type-check-exp env) e)
      (define recur (type-check-exp env))
      (match e
        [(Prim 'vector-proxy (list v reader-v writer-v))
         (define-values (v^ v-ty) (recur v))
         (define-values (reader-v^ rv-ty) (recur reader-v))
         (define-values (writer-v^ wv-ty) (recur writer-v))
         (match* (v-ty rv-ty wv-ty)
           [(`(Vector ,elems ...)
             `(Vector (,rd-srcs -> ,rd-tgts) ...)
             `(Vector (,wt-srcs -> ,wt-tgts) ...))
            (assert (and (equal? (length elems) (length rd-srcs))
                         (equal? (length rd-srcs) (length wt-srcs)))
                    "vector-proxy: arguments of different arities")
            (for ([elem elems] [rd-src rd-srcs] [rd-tgt rd-tgts] [wt-src wt-srcs] [wt-tgt wt-tgts])
              (check-type-equal? elem rd-src e)
              (check-type-equal? elem wt-tgt e)
              (check-type-equal? rd-tgt wt-src e))
            (values
              (Prim 'vector-proxy (list v^ reader-v^ writer-v^))
              `(Vector ,@rd-tgts))])]
             
        [(Prim 'raw-vector args)
         (define-values (e^ t) (recur (Prim 'vector args)))
         (match e^
           [(HasType (Prim 'vector args^) _)
            (values (Prim 'raw-vector args^) t)])]
